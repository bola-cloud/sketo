...
@section('content')
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="font-weight-bold text-white mb-1">{{ __('app.reports.title_date_range') }}</h2>
        ...
        <div class="premium-card bg-glass mb-4 p-4 d-print-none" style="border-radius: 15px;">
            <form action="{{ route('reports.dateRange') }}" method="GET">
                <div class="row align-items-end">
                    <div class="col-md-5">
                        <label class="text-white-50 font-weight-bold small">Start Date</label>
                        <input type="date" name="start_date" class="form-control form-control-lg bg-slate-800 text-white border-0" value="{{ request('start_date') }}" required style="border-radius: 10px;">
                    </div>
                    <div class="col-md-5">
                        <label class="text-white-50 font-weight-bold small">End Date</label>
                        <input type="date" name="end_date" class="form-control form-control-lg bg-slate-800 text-white border-0" value="{{ request('end_date') }}" required style="border-radius: 10px;">
                    </div>
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-primary btn-lg w-100" style="border-radius: 10px; font-weight: bold;"><i class="la la-search"></i> {{ __('app.common.search') }}</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <style>
        ...
        @media print {
            .btn-premium, .sidebar-wrapper, .header-navbar, .footer, .d-print-none { display: none !important; }
