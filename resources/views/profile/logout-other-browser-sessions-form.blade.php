<x-action-section>
    ...
    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('app.profile.browser_sessions_content') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span
                                            class="text-green-500 font-semibold">{{ __('app.profile.browser_sessions_device') }}</span>
                                    @else
                                        {{ __('app.profile.browser_sessions_last_active') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('app.profile.browser_sessions_log_out') }}
            </x-button>
        ...
        <x-dialog-modal wire:model.live="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('app.profile.browser_sessions_confirm_pass') }}

                <div class="mt-4" x-data="{}"
                    x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-input type="password" class="mt-1 block w-3/4" autocomplete="current-password"
                        placeholder="{{ __('app.profile.browser_sessions_password') }}" x-ref="password"
